...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    try:
        abbr = us_state_abbrev[state_name]
        return abbr
    except KeyError:
        return NOT_FOUND
...
def get_longest_state(data):
    ...
    # return list(filter(lambda x:len(x)==max_length, data))
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    result = list(sorted(us_state_abbrev.values()))[:10]
    result.extend(sorted(states)[-10:])
    return result