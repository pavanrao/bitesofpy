...
def check_bt(donor, recipient):
    ...
    """
    donor, recipient = get_numeric_bloodtype(donor), get_numeric_bloodtype(recipient)
    return -1 not in _particular_antigen_comp(donor, recipient)


def get_numeric_bloodtype(input):
    if type(input) is Bloodtype:
        return input.value
    elif type(input) is str:
        try:
            output = blood_type_text[input].value
            return output
        except KeyError:
            raise ValueError(f'{input} is invalid blood type value')
        ...
        raise TypeError(f'{input} is invalid type, accepted types: int | str | Bloodtype')
